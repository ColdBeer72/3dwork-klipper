...
install_hooks()
{
	...
	if [[ ! -e ~/printer_data/config/3dwork-klipper/.git/hooks/post-merge ]]
	then
 	   ln -s ~/printer_data/config/3dwork-klipper/scripts/3dwork-post-merge.sh ~/printer_data/config/3dwork-klipper/.git/hooks/post-merge
	fi
	if [[ ! -e ~/klipper/.git/hooks/post-merge ]]
	then
 	   ln -s ~/printer_data/config/3dwork-klipper/scripts/klipper-post-merge.sh ~/klipper/.git/hooks/post-merge
	fi
	if [[ ! -e ~/moonraker/.git/hooks/post-merge ]]
	then
 	   ln -s ~/printer_data/config/3dwork-klipper/scripts/moonraker-post-merge.sh ~/moonraker/.git/hooks/post-merge
	fi
}
...
pi  ALL=(ALL) NOPASSWD: ~/printer_data/config/3dwork-klipper/scripts/3dwork-update.sh
pi  ALL=(ALL) NOPASSWD: ~/printer_data/config/3dwork-klipper/scripts/klipper-mcu-update.sh
pi  ALL=(ALL) NOPASSWD: ~/printer_data/config/3dwork-klipper/scripts/moonraker-update.sh
#EOF
...
pi  ALL=(ALL) NOPASSWD: ~/printer_data/config/3dwork-klipper/scripts/change-hostname-as-root.sh
#EOF
